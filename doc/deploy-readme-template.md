...
- 첫 시작 안내의 장식 배트는 제거하고 목표 문구만 표시. 실제 타격 시의 배트 스윙은 유지.
- 존 안내는 은은한 그린빛 웨이브를 START ROUND 전까지 반복하고, 라운드 시작 즉시 종료. 시스템 모션 감소 설정에서는 정적인 강조로 표시.
- 목표 안내는 페이지 로드마다 처음 한 번 표시. 게임 시작 후 다음 라운드와 RESET에서는 숨기고, 새로고침하면 다시 표시.
...
- 투구·배트 스윙·타구는 OS 모션 감소 설정과 관계없이 재생. 게임 내 Normal / Quick 선택으로 속도 조절.

## 존 가이드와 모바일 입력

- START ROUND 이후 마우스·펜 호버 또는 키보드 포커스 시 반투명 배트와 타점을 표시. 실제 스윙 카메라와 배트 치수로 투영하며 확률·결과는 변경하지 않음.
- 터치 입력에서는 호버 가이드를 표시하지 않고, 한 번 터치하면 바로 스윙. 선택·라운드 종료·화면 크기 변경 시 가이드를 숨김.
- NEXT ROUND 이후 포커스는 START ROUND 버튼으로 이동. 베팅 금액은 직접 터치할 때만 편집.

## 테스트 항목

- 첫 버튼 조작 후 음악 재생. 인트로 15%, 경기 BGM 45%, 마스터 80%.
